...
static requiem_timer_t dh_param_regeneration_timer;
...
static int dh_check_elapsed(void)
{
        ...
        if ( ret < 0 ) {
                ...
                requiem_log(REQUIEM_LOG_ERR, "could not stat %s: %s.\n", DH_FILENAME, strerror(errno));
                return -1;
        }
...
static int dh_params_load(gnutls_dh_params dh, unsigned int req_bits)
{
        ...
        if ( ! fd ) {
                if ( errno != ENOENT )
                        requiem_log(REQUIEM_LOG_ERR, "could not open %s for reading: %s.\n", DH_FILENAME, strerror(errno));

                return -1;
        }

        ret = requiem_io_new(&pfd);
        if ( ret < 0 ) {
                fclose(fd);
                return -1;
        }

        requiem_io_set_file_io(pfd, fd);

        size = requiem_io_read_delimited(pfd, (void *) &bits);
        if ( size < 0 || size != sizeof(*bits) ) {
                requiem_perror(size, "error reading dh-prime length");
                goto err;
        }
        ...
        prime.size = size = requiem_io_read_delimited(pfd, &prime.data);
        if ( size < 0 ) {
                requiem_perror(size, "error reading dh-prime");
                goto err;
        }

        generator.size = size = requiem_io_read_delimited(pfd, &generator.data);
        if ( size < 0 ) {
                requiem_perror(size, "error reading dh generator");
                goto err;
        }

        ret = gnutls_dh_params_import_raw(dh, &prime, &generator);
        if ( ret < 0 )
                requiem_log(REQUIEM_LOG_WARN, "error importing Diffie-Hellman parameters: %s.\n", gnutls_strerror(ret));

        free(bits);
        free(prime.data);
        free(generator.data);
        requiem_io_close(pfd);
        requiem_io_destroy(pfd);
...
static int dh_params_save(gnutls_dh_params dh, unsigned int dh_bits)
{
        ...
        if ( ret < 0 ) {
                requiem_log(REQUIEM_LOG_WARN, "error exporting Diffie-Hellman parameters: %s.\n", gnutls_strerror(ret));
                return -1;
        }

        fd = open(DH_FILENAME, O_CREAT|O_TRUNC|O_WRONLY, S_IRUSR|S_IWUSR);
        if ( fd < 0 ) {
                requiem_log(REQUIEM_LOG_ERR, "error opening %s for writing: %s.\n", DH_FILENAME, strerror(errno));
                free(prime.data);
                free(generator.data);
                return -1;
        }
        ...
        requiem_io_set_sys_io(pfd, fd);
        requiem_io_write_delimited(pfd, &dh_bits, sizeof(dh_bits));
        requiem_io_write_delimited(pfd, prime.data, prime.size);
        requiem_io_write_delimited(pfd, generator.data, generator.size);
...
static void dh_params_regenerate(void *data)
{
        ...
        if ( ret < 0 ) {
                requiem_log(REQUIEM_LOG_WARN, "error initializing dh parameters object: %s.\n", gnutls_strerror(ret));
                return;
        }
        ...
        requiem_log(REQUIEM_LOG_INFO, "Regenerated %d bits Diffie-Hellman key for TLS.\n", global_dh_bits);

        dh_params_save(cur_dh_params, global_dh_bits);
        requiem_timer_set_expire(&dh_param_regeneration_timer, global_dh_lifetime);
        requiem_timer_reset(&dh_param_regeneration_timer);
}
...
static int get_params(gnutls_session session, gnutls_params_type type, gnutls_params_st *st)
{
        ...
        if ( ret < 0 ) {
                requiem_log(REQUIEM_LOG_WARN, "error creating a new dh parameters object: %s.\n", gnutls_strerror(ret));
                return -1;
        }
        ...
        if ( ret < 0 ) {
                requiem_log(REQUIEM_LOG_WARN, "could not copy dh params for sessions: %s.\n", gnutls_strerror(ret));
...
static int handle_gnutls_error(requiem_io_t *pio, gnutls_session session, server_generic_client_t *client, int ret,
                               gnutls_alert_description *alert_desc)
{
        ...
        if ( ret == GNUTLS_E_AGAIN ) {
                ...
                server_generic_log_client(client, REQUIEM_LOG_WARN, "TLS alert from client: %s.\n", alert);
                return 1;
        }

        else if ( ret == GNUTLS_E_FATAL_ALERT_RECEIVED ) {
                alert = gnutls_alert_get_name(gnutls_alert_get(session));
                server_generic_log_client(client, REQUIEM_LOG_WARN, "TLS fatal alert from client: %s.\n", alert);
        }

        else {
                server_generic_log_client(client, REQUIEM_LOG_WARN, "TLS error: %s.\n", gnutls_strerror(ret));
...
static int verify_certificate(server_generic_client_t *client, gnutls_session session, gnutls_alert_description *alert)
{
        ...
        time_t now;
        unsigned int status;
        requiem_log_t pri = REQUIEM_LOG_WARN;
                                           ...
                                           uint64_t *analyzerid, requiem_connection_permission_t *permission)
{
        ...
        if ( ! cert_list || cert_list_size != 1 ) {
                server_generic_log_client(client, REQUIEM_LOG_WARN, "invalid number of peer certificate: %d.\n",
                                          cert_list_size);
                return -1;
        }
        ...
        if ( ret < 0 ) {
                server_generic_log_client(client, REQUIEM_LOG_ERR, "error initializing tls certificate: %s.\n",
                                          gnutls_strerror(ret));
                return -1;
        }
        ...
        if ( ret < 0) {
                server_generic_log_client(client, REQUIEM_LOG_ERR, "error importing certificate: %s.\n",
                                          gnutls_strerror(ret));
                goto err;
        }
        ...
        if ( ret < 0 ) {
                server_generic_log_client(client, REQUIEM_LOG_WARN, "error getting certificate DN: %s.\n",
                                          gnutls_strerror(ret));
                goto err;
        }
        ...
        if ( ret != 1 ) {
                ret = -1;
                server_generic_log_client(client, REQUIEM_LOG_WARN, "error parsing certificate DN.\n");
                goto err;
        }

        ret = gnutls_x509_crt_get_dn_by_oid(cert, GNUTLS_OID_X520_COMMON_NAME, 0, 0, buf, &size);
        if ( ret < 0 ) {
                server_generic_log_client(client, REQUIEM_LOG_WARN, "error getting certificate permission: %s.\n",
                                          gnutls_strerror(ret));
                goto err;
        }
        ...
        if ( ret != 1 ) {
                ret = -1;
                server_generic_log_client(client, REQUIEM_LOG_WARN, "error parsing certificate permission.\n");
                goto err;
        }
...
int manager_auth_client(server_generic_client_t *client, requiem_io_t *pio, gnutls_alert_description *alert)
{
         ...
         */
        session = requiem_io_get_fdptr(pio);
        if ( ! session ) {
                union { int fd; void *ptr; } data;

                ret = gnutls_init(&session, GNUTLS_SERVER);
                if ( ret < 0 ) {
                        server_generic_log_client(client, REQUIEM_LOG_WARN, "error initializing TLS session: %s.\n",
                                                  gnutls_strerror(ret));
                        return -1;
                }
                ...
                requiem_io_set_tls_io(pio, session);
        }
...
int manager_auth_disable_encryption(server_generic_client_t *client, requiem_io_t *pio)
{
        ...
        requiem_io_set_sys_io(pio, requiem_io_get_fd(pio));
...
static int tls_priority_init(const char *tlsopts)
{
        ...
        if ( ret < 0 ) {
                requiem_log(REQUIEM_LOG_ERR, "TLS priority error: %s: '%s'.\n", gnutls_strerror(ret), errptr);
                return -2;
        }
#else
        if ( tlsopts ) {
                requiem_log(REQUIEM_LOG_ERR, "settings TLS options require GnuTLS 2.2.0 or above.\n");
                return -2;
        }
#endif
...
int manager_auth_init(requiem_client_t *client, const char *tlsopts, int dh_bits, int dh_lifetime)
{
        int ret;
        char keyfile[PATH_MAX], certfile[PATH_MAX], crlfile[PATH_MAX];
        requiem_client_profile_t *cp = requiem_client_get_profile(client);
        ...
        gcry_control(GCRYCTL_SET_THREAD_CBS, &gcry_threads_requiem);
        gnutls_global_init();
        ...
        requiem_client_profile_get_tls_server_keycert_filename(cp, certfile, sizeof(certfile));

        ret = access(certfile, R_OK);
        if ( ret < 0 ) {
                requiem_log(REQUIEM_LOG_ERR, "could not open %s for reading.\n", certfile);
        ...
        if ( ret < 0 ) {
                requiem_log(REQUIEM_LOG_ERR, "%s: %s\n", keyfile, gnutls_strerror(ret));
                return -1;
        }

        requiem_client_profile_get_tls_server_ca_cert_filename(cp, certfile, sizeof(certfile));
        ...
        if ( ret < 0 ) {
                requiem_log(REQUIEM_LOG_ERR, "could not open %s for reading/writing.\n", MANAGER_RUN_DIR);
                return requiem_error_from_errno(errno);
        }
        ...
        if ( ret != -1 && dh_params_load(cur_dh_params, dh_bits) == 0 )
                requiem_timer_set_expire(&dh_param_regeneration_timer, dh_lifetime - ret);
        else {
                requiem_log(REQUIEM_LOG_INFO, "Generating %d bits Diffie-Hellman key for TLS...\n", dh_bits);

                gnutls_dh_params_generate2(cur_dh_params, dh_bits);
                dh_params_save(cur_dh_params, dh_bits);

                requiem_timer_set_expire(&dh_param_regeneration_timer, dh_lifetime);
        }

        gnutls_certificate_set_params_function(cred, get_params);

        if ( dh_lifetime ) {
                requiem_timer_set_callback(&dh_param_regeneration_timer, dh_params_regenerate);
                requiem_timer_init(&dh_param_regeneration_timer);
        }
