...
typedef struct {
        ...
        requiem_bool_t we_connected;
        requiem_list_t write_msg_list;
...
int sensor_server_broadcast_admin_command(uint64_t *analyzerid, requiem_msg_t *msg);

int sensor_server_add_client(server_generic_t *server, server_generic_client_t **client, requiem_connection_t *cnx);

int sensor_server_write_client(server_generic_client_t *dst, requiem_msg_t *msg);
