...
static REQUIEM_LIST(message_queue);
static gl_lock_t queue_list_mutex = gl_lock_initializer;

static requiem_bool_t input_available = FALSE;
...
static int process_message(requiem_msg_t *msg)
{
        int ret;
        ...
        if ( ret < 0 ) {
                 ...
                 */
                requiem_log(REQUIEM_LOG_ERR, "Invalid message received.\n");
                return ret;
        }

        /*
         * requiem-msg is usefull for report plugin failover.
         * We don't need to call requiem_msg_destroy(), as
...
static size_t read_message_scheduled_from_pool(bufpool_t *pool, size_t count)
{
        ...
        while ( count-- ) {
                requiem_return_val_if_fail(bufpool_get_message(pool, &msg) == 1, proc);
...
static void read_message_scheduled(idmef_queue_t *queue)
{
        ...
        requiem_return_if_fail(total == 0);
}
...
static void schedule_queued_message(struct timespec *last_wakeup)
{
        ...
        do {
                ...
                while ( 1 ) {
                        gl_lock_lock(queue_list_mutex);
                        queue = requiem_list_get_next_safe(&message_queue, queue, bkp, idmef_queue_t, list);
                        gl_lock_unlock(queue_list_mutex);
...
static void *message_reader(void *arg)
{
        ...
        if ( ret < 0 ) {
                requiem_log(REQUIEM_LOG_ERR, "couldn't set thread signal mask.\n");
                return NULL;
        }
...
int idmef_message_schedule(idmef_queue_t *queue, requiem_msg_t *msg)
{
        ...
        switch (requiem_msg_get_priority(msg)) {

        case REQUIEM_MSG_PRIORITY_HIGH:
                mqueue = queue->high;
                break;

        case REQUIEM_MSG_PRIORITY_MID:
                mqueue = queue->mid;
                break;
...
idmef_queue_t *idmef_message_scheduler_queue_new(requiem_client_t *client)
{
        ...
        id = get_unique_id();
        requiem_client_profile_get_backup_dirname(requiem_client_get_profile(client), bdir, sizeof(bdir));

        snprintf(buf, sizeof(buf), "%s/high-buffer.%" REQUIEM_PRIu64, bdir, id);
        ...
        snprintf(buf, sizeof(buf), "%s/medium-buffer.%" REQUIEM_PRIu64, bdir, id);
        ...
        snprintf(buf, sizeof(buf), "%s/low-buffer.%" REQUIEM_PRIu64, bdir, id);
        ...
        gl_lock_lock(queue_list_mutex);
        requiem_list_add_tail(&message_queue, &queue->list);
        gl_lock_unlock(queue_list_mutex);
...
static int flush_failover(requiem_failover_t *failover, const char *name)
{
        int ret;
        requiem_msg_t *msg;
        ...
        requiem_log(REQUIEM_LOG_INFO, "%s: flushing %lu buffered messages from a previous run.\n",
                    name, available);

        do {
                ret = requiem_failover_get_saved_msg(failover, &msg);
                if ( ret <= 0 )
                        break;
...
static int del_cb(const char *filename, const struct stat *st, int flag)
{
        ...
        ret = unlink(filename);
        return ( ret < 0 && errno != EISDIR ) ? requiem_error_from_errno(errno) : 0;
}
...
static int failover_unlink(const char *dirname)
{
        ...
        ret = rmdir(dirname);
        return (ret < 0) ? requiem_error_from_errno(errno) : 0;
}
...
int idmef_message_scheduler_init(void)
{
        ...
        requiem_client_profile_get_backup_dirname(requiem_client_get_profile(manager_client), bdir, sizeof(bdir));

        dir = opendir(bdir);
        if ( ! dir ) {
                requiem_log(REQUIEM_LOG_ERR, "error opening directory '%s': %s.\n", bdir, strerror(errno));
                return -1;
        }
        ...
        while ( (de = readdir(dir)) ) {
                ...
                ret = requiem_failover_new(&failover, filename);
                if ( ret < 0 )
                        return ret;

                flush_failover(failover, de->d_name);
                requiem_failover_destroy(failover);

                ret = failover_unlink(filename);
                if ( ret < 0 )
                        requiem_log(REQUIEM_LOG_ERR, "couldn't remove failover '%s': %s.\n", filename, requiem_strerror(ret));
        }
        ...
        if ( ret < 0 )
                requiem_log(REQUIEM_LOG_ERR, "couldn't create message processing thread.\n");
...
void idmef_message_scheduler_exit(void)
{
        ...
        requiem_log(REQUIEM_LOG_INFO, "Waiting queued message to be processed.\n");
        gl_thread_join(thread, NULL);

        gl_cond_destroy(input_cond);
        gl_lock_destroy(input_mutex);

        requiem_list_for_each_safe(&message_queue, tmp, bkp) {
                queue = requiem_list_entry(tmp, idmef_queue_t, list);
                queue_destroy(queue);
        }
}
...
{
        int ret = 0;
        requiem_bool_t relay_filter_available = 0;
