/*****
*
* Copyright (C) 1999-2007,2008 PreludeIDS Technologies. All Rights Reserved.
...
#include <librequiem/requiem-io.h>
#include <librequiem/requiem-msg.h>
...
static int send_auth_result(server_generic_client_t *client, int result)
{
        ...
        if ( ! client->msg ) {
                ret = requiem_msg_new(&client->msg, 1, sizeof(uint64_t), REQUIEM_MSG_AUTH, 0);
                if ( ret < 0 )
                        return -1;

                cp = requiem_client_get_profile(manager_client);
                nident = requiem_hton64(requiem_client_profile_get_analyzerid(cp));
                requiem_msg_set(client->msg, result, sizeof(nident), &nident);
        }

        ret = requiem_msg_write(client->msg, client->fd);
        ...
        if ( ret < 0 ) {
                ...
                requiem_msg_destroy(client->msg);
                return -1;
        }
...
static int send_queued_alert(server_generic_client_t *client)
{
        ...
        do {
                ret = gnutls_alert_send(requiem_io_get_fdptr(client->fd), GNUTLS_AL_FATAL, client->alert);
        ...
        client->alert = 0;
        gnutls_deinit(requiem_io_get_fdptr(client->fd));
        requiem_io_set_sys_io(client->fd, requiem_io_get_fd(client->fd));
...
/*
 * Read the message sent by the Requiem Manager client.
...
static int authenticate_client(server_generic_t *server, server_generic_client_t *client)
{
        ...
        if ( ! client->msg && ! client->alert && ! (client->state & SERVER_GENERIC_CLIENT_STATE_AUTHENTICATED) ) {
                ...
                ret = send_auth_result(client, REQUIEM_MSG_AUTH_SUCCEED);
                if ( ret != 1 )
                        return ret;
        }
        ...
        if ( server->sa->sa_family == AF_UNIX && ! (client->state & SERVER_GENERIC_CLIENT_STATE_ACCEPTED) ) {
                ...
                server_generic_log_client(client, REQUIEM_LOG_INFO, "disabled encryption on local UNIX connection.\n");
        }
...
static int do_close_fd(server_generic_client_t *client)
{
        ...
        do {
                ret = requiem_io_close(client->fd);
                if ( ret == 0 )
                        break;

                else if ( ret < 0 && requiem_io_is_error_fatal(client->fd, ret) )
                        return 0;

                if ( requiem_error_get_code(ret) == REQUIEM_ERROR_EAGAIN ) {
                        fd_ptr = requiem_io_get_fdptr(client->fd);
                ...
                server_generic_log_client(client, REQUIEM_LOG_WARN, "connection closure error: %s.\n", requiem_strerror(ret));
...
static int close_connection_cb(server_generic_client_t *client)
{
        ...
        if ( client->fd ) {
                ...
                requiem_io_destroy(client->fd);
                server_generic_log_client(client, REQUIEM_LOG_INFO, "closing connection.\n");
        }
...
static int tcpd_auth(server_generic_client_t *cdata, int clnt_sock)
{
        ...
        request_init(&request, RQ_DAEMON, "requiem-manager", RQ_FILE, clnt_sock, 0);

        fromhost(&request);

        ret = hosts_access(&request);
        if ( ! ret ) {
                server_generic_log_client(cdata, REQUIEM_LOG_WARN, "tcp wrapper refused connection.\n");
                return -1;
        }
 ...
 * create a requiem_io object for IO abstraction.
 *
                               ...
                               server_generic_client_t *cdata, int client)
{
        ...
        ret = requiem_io_new(&cdata->fd);
        if ( ret < 0 )
                return ret;

        requiem_io_set_sys_io(cdata->fd, client);
...
static int accept_connection(server_generic_t *server, server_generic_client_t *cdata)
{
        ...
        if ( sock < 0 ) {
                requiem_log(REQUIEM_LOG_ERR, "accept error: %s.\n", strerror(errno));
                return -1;
        }

        ret = setsockopt(sock, SOL_SOCKET, SO_KEEPALIVE, &on, sizeof(int));
        if ( ret < 0 )
                requiem_log(REQUIEM_LOG_ERR, "could not set SO_KEEPALIVE socket option: %s.\n", strerror(errno));
...
static int handle_connection(server_generic_t *server)
{
        ...
        if ( ret < 0 ) {
                requiem_log(REQUIEM_LOG_ERR, "queueing client FD for server logic processing failed.\n");
                requiem_io_close(cdata->fd);
                requiem_io_destroy(cdata->fd);
                free(cdata);
                return -1;
        }
...
/*
 * Wait for client to connect on the Requiem Manager.
 */
...
static int generic_server(int sock, struct sockaddr *addr, size_t alen)
{
        ...
        if ( ret < 0 )
                return requiem_error_verbose(requiem_error_code_from_errno(errno),
                                             "could not bind socket: %s", strerror(errno));

        ret = listen(sock, 10);
        if ( ret < 0 )
                return requiem_error_verbose(REQUIEM_ERROR_GENERIC, "listen error: %s", strerror(errno));
...
static int is_unix_socket_already_used(int sock, struct sockaddr_un *sa, int addrlen)
{
        ...
        if ( ! S_ISSOCK(st.st_mode) )
                return requiem_error_verbose(REQUIEM_ERROR_GENERIC, "'%s' exist and is not an UNIX socket", sa->sun_path);

        ret = connect(sock, (struct sockaddr *) sa, addrlen);
        if ( ret == 0 )
                return requiem_error_verbose(REQUIEM_ERROR_GENERIC, "UNIX socket '%s' already in use", sa->sun_path);
        ...
        if ( ret < 0 )
                return requiem_error_verbose(REQUIEM_ERROR_GENERIC, "could not delete unused UNIX socket: %s", strerror(errno));
...
static int unix_server_start(server_generic_t *server)
{
        ...
        if ( server->sock < 0 )
                return requiem_error_verbose(REQUIEM_ERROR_GENERIC, "error creating UNIX socket: %s", strerror(errno));
        ...
        if ( ret < 0 )
                return requiem_error_verbose(REQUIEM_ERROR_GENERIC, "could not set permission on UNIX socket: %s", strerror(errno));
...
static int inet_server_start(server_generic_t *server, struct sockaddr *addr, socklen_t addrlen)
{
        ...
        if ( server->sock < 0 )
                return requiem_error_verbose(REQUIEM_ERROR_GENERIC, "error creating socket: %s", strerror(errno));

        ret = setsockopt(server->sock, SOL_SOCKET, SO_REUSEADDR, &on, sizeof(int));
        if ( ret < 0 ) {
                ret = requiem_error_verbose(REQUIEM_ERROR_GENERIC, "error setting SO_REUSEADDR: %s", strerror(errno));
                goto err;
        }
...
#if ! ((defined _WIN32 || defined __WIN32__) && !defined __CYGWIN__)
static requiem_bool_t is_unix_addr(const char **out, const char *addr)
{
        int ret;
        const char *ptr;
        ...
        ptr = strchr(addr, ':');
        *out = (ptr && *(ptr + 1)) ? ptr + 1 : requiem_connection_get_default_socket_filename();
...
static int do_getaddrinfo(struct addrinfo **ai, const char *addr, unsigned int port)
{
        ...
        if ( ret != 0 )
                ret = requiem_error_verbose(REQUIEM_ERROR_GENERIC, "could not resolve '%s': %s",
...
static int sg_bind_common(server_generic_t *server, unsigned int port)
{
        ...
        if ( server->sa->sa_family == AF_UNIX )
                requiem_log(REQUIEM_LOG_INFO, "server started (listening on %s).\n",
                            ((struct sockaddr_un *) server->sa)->sun_path);
        else {
#endif
                in_addr = requiem_sockaddr_get_inaddr(server->sa);
                assert(in_addr);

                inet_ntop(server->sa->sa_family, in_addr, out, sizeof(out));
                requiem_log(REQUIEM_LOG_INFO, "server started (listening on %s port %u).\n", out, port);
#if ! ((defined _WIN32 || defined __WIN32__) && !defined __CYGWIN__)
        }
#endif
...
void server_generic_notify_write_enable(server_generic_client_t *client)
{
        ...
        ev_io_set(&client->evio, (int) requiem_io_get_fd(client->fd), EV_READ|EV_WRITE);
...
{
        ev_io_stop(manager_event_loop, &client->evio);
        ev_io_set(&client->evio, (int) requiem_io_get_fd(client->fd), EV_READ);
        ev_io_start(manager_event_loop, &client->evio);
}
...
int server_generic_process_requests(server_generic_t *server, server_generic_client_t *client)
{
        ...
        ev_io_init(&client->evio, libev_notification_cb, (int) requiem_io_get_fd(client->fd), EV_READ);
        ev_io_start(manager_event_loop, &client->evio);
...
void server_generic_log_client(server_generic_client_t *cnx, requiem_log_t priority, const char *fmt, ...)
{
        va_list ap;
        int ret = 0;
...
#endif
                in_addr = requiem_sockaddr_get_inaddr((struct sockaddr *) &cnx->sa);
                if ( ! in_addr )
                        goto out;
        ...
        if ( cnx->ident && cnx->permission ) {
                ret = requiem_string_sprintf(out, " 0x%" REQUIEM_PRIx64, cnx->ident);
                if ( ret < 0 )
                        goto out;

                ret = requiem_connection_permission_to_string(cnx->permission, out);
        }
        ...
        va_start(ap, fmt);
        ret = requiem_string_vprintf(out, fmt, ap);
        va_end(ap);

        requiem_log(priority, "[%s%s", addr, requiem_string_get_string(out));
...
int server_generic_client_set_permission(server_generic_client_t *client, requiem_connection_permission_t permission)
{
