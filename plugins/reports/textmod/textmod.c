...
int textmod_LTX_requiem_plugin_version(void);
int textmod_LTX_manager_plugin_init(requiem_plugin_entry_t *pe, void *rootopt);
...
REQUIEM_PLUGIN_OPTION_DECLARE_STRING_CB(textmod, textmod_plugin_t, logfile)
...
static void print_string(textmod_plugin_t *plugin, int depth, const char *format, requiem_string_t *string)
{
	const char *content;
...
static void process_time(textmod_plugin_t *plugin, const char *type, idmef_time_t *time) 
{
	...
	if ( ! localtime_r(&t, &tm) ) {
                requiem_log(REQUIEM_LOG_ERR, "error converting timestamp to local time.\n");
                return;
        }

        len = strftime(time_human, sizeof(time_human), "%Y-%m-%d %H:%M:%S", &tm);
        if ( len == 0 ) {
                requiem_log(REQUIEM_LOG_ERR, "error converting UTC time to string.\n");
                return;
        }
        ...
        ret = requiem_string_new(&ntpstamp);
        if ( ret < 0 ) {
        ...
        print(plugin, 0, "%s: %s (%s)\n", type, requiem_string_get_string(ntpstamp), time_human);
        requiem_string_destroy(ntpstamp);
}
...
static void process_process(textmod_plugin_t *plugin, int depth, idmef_process_t *process)
{
	...
	while ( (string = idmef_process_get_next_arg(process, string)) ) {
		...
		print(plugin, depth, "%s ", requiem_string_get_string(string));
	}
...
static void process_web_service(textmod_plugin_t *plugin, idmef_web_service_t *web) 
{
	...
	while ( (string = idmef_web_service_get_next_arg(web, string)) ) {
		...
		print(plugin, 0, "%s ", requiem_string_get_string(string));
	}
...
static void process_file_access(textmod_plugin_t *plugin, int depth, idmef_file_access_t *file_access) 
{
	...
	while ( (permission = idmef_file_access_get_next_permission(file_access, permission)) ) {
		...
		print(plugin, depth, "%s ", requiem_string_get_string(permission));
	}
...
static void process_data(textmod_plugin_t *plugin, idmef_additional_data_t *ad) 
{
	...
	if ( requiem_string_get_len(out) <= 80 )
                print(plugin, 0, " %s\n", requiem_string_get_string(out));
        else
                print(plugin, 0, "\n%s\n", requiem_string_get_string(out));
...
static int textmod_run(requiem_plugin_instance_t *pi, idmef_message_t *message) 
{
        textmod_plugin_t *plugin = requiem_plugin_instance_get_plugin_data(pi);
        ...
        default:
                requiem_log(REQUIEM_LOG_WARN, "unknow message type: %d.\n", idmef_message_get_type(message));
                break;
        }
...
static int textmod_init(requiem_plugin_instance_t *pi, requiem_string_t *out)
{
        FILE *fd;
        textmod_plugin_t *plugin = requiem_plugin_instance_get_plugin_data(pi);
        ...
        if ( ! plugin->logfile ) {
                ...
                if ( ! fd ) {
                        requiem_string_sprintf(out, "error opening '%s' in append mode", plugin->logfile);
                        return -1;
                }
        }
...
static int textmod_activate(requiem_option_t *opt, const char *optarg, requiem_string_t *err, void *context) 
{
...
static void textmod_destroy(requiem_plugin_instance_t *pi, requiem_string_t *err)
{
        textmod_plugin_t *plugin = requiem_plugin_instance_get_plugin_data(pi);
...
int textmod_LTX_manager_plugin_init(requiem_plugin_entry_t *pe, void *rootopt)
{
	...
	requiem_option_t *opt;
        ...
        ret = requiem_option_add(rootopt, &opt, hook, 0, "textmod", "Option for the textmod plugin",
                                 REQUIEM_OPTION_ARGUMENT_OPTIONAL, textmod_activate, NULL);
        if ( ret < 0 )
                return ret;

        requiem_plugin_set_activation_option(pe, opt, textmod_init);
        
        ret = requiem_option_add(opt, NULL, hook, 'l', "logfile", "Specify logfile to use",
                                 REQUIEM_OPTION_ARGUMENT_REQUIRED,
                                 textmod_set_logfile, textmod_get_logfile);
        if ( ret < 0 )
                return ret;
        
        requiem_plugin_set_name(&textmod_plugin, "TextMod");
        requiem_plugin_set_destroy_func(&textmod_plugin, textmod_destroy);
        manager_report_plugin_set_running_func(&textmod_plugin, textmod_run);

        requiem_plugin_entry_set_plugin(pe, (void *) &textmod_plugin);
