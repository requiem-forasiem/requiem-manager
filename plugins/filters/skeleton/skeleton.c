...
#include <librequiem/list.h>
#include <librequiem/idmef-tree.h>
...
static int set_skeleton_hook(requiem_option_t *opt, const char *arg) 
{
        int i, ret;
...
static int set_skeleton_rule(requiem_option_t *opt, const char *arg) 
{
...
static int enable_skeleton(requiem_option_t *opt, const char *arg) 
{
        if ( ! tbl_index )
                return requiem_option_success;
...
plugin_generic_t *plugin_init(int argc, char **argv)
{
        ...
        opt = requiem_option_add(NULL, CLI_HOOK|CFG_HOOK|WIDE_HOOK, 0, "skeleton",
                                 "Option for the skeleton plugin", no_argument,
                                 enable_skeleton, NULL);

        requiem_option_add(opt, CLI_HOOK|CFG_HOOK|WIDE_HOOK, 'r', "rule",
                                 "Filtering rule", required_argument,
                                 set_skeleton_rule, NULL);
        
        requiem_option_add(opt, CLI_HOOK|CFG_HOOK|WIDE_HOOK, 'h', "hook",
        ...
        plugin_set_contact(&plugin, "yoannv@gmail.com");
